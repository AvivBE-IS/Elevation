...
//ex1 : Map the data differently so you are left with an array of
//each user's email and their company's name. Your resulting array should look like this:

userEmailCompany = users.map((u) => {
  return { email: u.email, companyName: u.company.name };
});
//console.log(userEmailCompany);
...
zipCodeStartsWithFive = users.filter(
  (u) => u.address.zipcode.slice(0, 1) === "5"
);
...
zipCodeStartsWithFiveArray = users
  .filter((u) => u.address.zipcode.slice(0, 1) === "5")
  .map((u) => u.id);
//console.log(zipCodeStartsWithFiveArray);
...
nameStartWithC = s.filter((n) => n.startsWith("C"));
